...
describe('UniswapV2Pair', () => {
  ...
  ].map(a => a.map(n => (typeof n === 'string' ? ethers.utils.bigNumberify(n) : expandTo18Decimals(n))))
  ...
  it('swap:token0', async () => {
    ...
    const expectedOutputAmount = ethers.utils.bigNumberify('1662497915624478906')
    await token0.transfer(pair.address, swapAmount)
  ...
  it('swap:token1', async () => {
    ...
    const swapAmount = expandTo18Decimals(1)
    const expectedOutputAmount = ethers.utils.bigNumberify('453305446940074565')
  ...
  it('feeTo:off', async () => {
    ...
    const swapAmount = expandTo18Decimals(1)
    const expectedOutputAmount = ethers.utils.bigNumberify('996006981039903216')
    await token1.transfer(pair.address, swapAmount)
  ...
  it('feeTo:on', async () => {
    ...
    expect(await token0.balanceOf(pair.address)).to.eq(ethers.utils.bigNumberify(1000).add('249501683697445'))
    expect(await token1.balanceOf(pair.address)).to.eq(ethers.utils.bigNumberify(1000).add('250000187312969'))
  })
})
